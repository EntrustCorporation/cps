...
def get_markdown_headers_from_url(url):
    ...
    for line in response.text.split('\n'):
        if line.startswith('#'):
            ...
            if len(parts) == 2:
                section_number, title = parts[0], parts[1]
                section_number = section_number.rstrip('.')
                sections[section_number] = title
        ...
        if line.startswith('#'):
            ...
            if len(parts) == 2:
                ...
                if header_level != section_level:
                    out.write(
                        f"! Section {section_number} (level {section_level}) does not match header level {header_level}\n", Fore.RED)
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        ...
        if doc_type in doc_types:
            if doc_type == "tlsbr":
                doc_types['tlsbr'] = get_markdown_headers_from_url(
                    "https://raw.githubusercontent.com/cabforum/servercert/main/docs/BR.md")
                
            if doc_type == "tlsevg":
                doc_types['tlsevg'] = get_markdown_headers_from_url(
                    "https://raw.githubusercontent.com/cabforum/servercert/main/docs/EVG.md")
